...
const Header = ({ themeChange, open, handleClick }) => {
  return (
    <Navbar
      fluid
      rounded
      className=" py-3.5 sticky top-0 left-0
    ...
    >
      ...
      <div className="flex md:order-2">
        <div className="flex item-center gap-4 justify-between">
          ...
          <Navbar.Toggle className=" !bg-inherit" />
        </div>
      </div>
      <Navbar.Collapse>
        <Navbar.Link
          onClick={handleClick}
          href="#about"
          className="text-smallTextColor dark:text-smallTextColorDark font-[800] text-[17px] md:mr-10 !bg-inherit"
        >
          About
        </Navbar.Link>
        <Navbar.Link
